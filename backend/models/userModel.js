...
const userSchema= new mongoose.Schema({
    ...
    //  default:"" --> So whenever new user will be created then one default image will be provided for default user profile picture later user can changed thier profile picture
    address:{
        type:Object,
        default:{line1:'', line2:''}
    },
...
// Now we have created the user schema. using this schema we will create one user model.

const userModel= mongoose.models.user || mongoose.model('user', userSchema)
// 'user' is a model name.
//  mongoose.model('user', doctorSchema) ->> It will create the user model, so whenever our project get start  this statment will be get excauted "mongoose.model('user', userSchema)". Then it will create the model for multile times(Which is bad, we want only one time model should be created not multiple time), so to prevent that add logic.

// mongoose.models.user ||  ---> so here we will checking models.user is available then we are going to use that model, and if it not available then we will create this model using this userSchema "mongoose.model('user', userSchema)".
